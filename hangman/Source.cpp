...
int letsplay()
{
	...
	while (1) //noooooooo
	{
		found = false;
		char key;
		printf("\n\nInsert letter (You DON'T need to press 'Enter' button):\n");
		A: c = _getch();
		if (c == 27) {
		printf("You pressed 'esc' button. \n");
		return pts;
		}
		else { putchar(c);  Sleep(QUARTER_SECOND); }
		if (c >= 'A' && c <= 'Z') c += 32;
		if (!((c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z') || c==13)) { system("cls"); hangman(true);
		printf("Wrong: %s\n\n%s\n", wrong, tmp); 
		printf("\nUnrecognized character!"); continue;
		}if (c == 13) goto A;
		strchr(array[i], c) ? found = true : found = false;
		if (strchr(wrong, c)) { system("cls"); hangman(true); printf("Wrong: %s\n\n%s\n", wrong, tmp); printf("\nCharacter '%c' already exists!!! \n", c); continue; }
		if (!found) {
